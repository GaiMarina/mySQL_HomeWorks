...
-- ------------------------------------------------
/*
CREATE DATABASE 4_SQL_HomeWork;
...
/*
YEAR() выделяет из даты год
RIGHT() — пять крайних справа символов, представляющих календарный день (MM-DD). 
Часть выражения, сравнивающая даты, выдает 1 или 0, 
что позволяет уменьшить результат на единицу, 
если текущий день (CURRENT_DATE) находится к началу календаря ближе, 
нежели день рождения пользователя.
Функция TIMESTAMPDIFF возвращает количество дней, месяцев или лет 
(в зависимости от того, что указано) между двумя датами.
Если указан параметр YEAR, функция возвращает количество лет между двумя датами.

(YEAR(CURRENT_DATE) - YEAR(profiles.birthday)) - 
...
-- Общее количество лайков до 12 лет.
SELECT COUNT(*) 'Количество лайков до 12 лет'
FROM likes
WHERE user_id IN 
	(SELECT user_id FROM `profiles`
    WHERE (TIMESTAMPDIFF(YEAR, profiles.birthday, curdate()) < 12) = 1);

-- Расписаны лайки у кого какие до 12 лет.
SELECT users.id,
	   CONCAT(users.firstname, ' ', users.lastname) AS 'Имя, Фамилия',
	   profiles.birthday AS 'День рождения', 
	   TIMESTAMPDIFF(YEAR, profiles.birthday, curdate()) AS 'Возраст',
...
HAVING (TIMESTAMPDIFF(YEAR, profiles.birthday, curdate()) < 12) = 1
...
SELECT
    ...
    WHERE messages.from_user_id = users.id) AS 'Имя, Фамилия',
